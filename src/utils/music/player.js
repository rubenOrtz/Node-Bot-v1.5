    ...
    client.manager = new Manager({
      ...
      .on("trackStart", async (player, track) => {
        ...
        const embed = new Discord.MessageEmbed()
          .setDescription(
            `${client.language.PLAYING} **[${track.title}](${
              track.uri
            })** [${formatDuration(client, track.duration)}] • <@${
        ...
        if (!player.stayInVc) {
          await client.shard
            ...
            .then(async (channel) => {
              ...
              await client.shard
                .broadcastEval(
                  async (c, { voiceChannel, msgID, textChannel }) => {
          ...
          if (track) {
            ...
            fetch(
              `http://${process.env.IP}:${process.env.bot1Port}/api/v1/247automix`,
              {
                method: "POST",
                ...
                await client.shard
                  ...
                  .then(async (channel) => {
                    ...
                    await client.shard.broadcastEval(
                      async (
                        c,
                        { voiceChannel, msgID, textChannel, resEmbed }
                      ) => {
                        const { MessageEmbed } = require("discord.js");
                        const channel = c.channels.cache.get(voiceChannel);
                        ...
                        if (c.channels.cache.get(textChannel)) {
                          const channel = c.channels.cache.get(textChannel);
