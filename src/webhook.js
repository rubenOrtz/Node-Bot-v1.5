...
app.listen(PORT, () =>
  console.log(`It's alive on http://${process.env.IP}:${PORT}`)
);
...
app.post("/api/v1/new_player", async (req, res) => {
  await client.shard
    .broadcastEval(
      async (client, { req_body }) => {
        const { MessageEmbed, Discord, WebhookClient } = require("discord.js");
        ...
        if (
          playerCanal.id != req_body[0]["channel"] &&
...
app.post("/api/v1/sendtwitchmessage", async (req, res) => {
  ...
  TwitchModel.findOne({
    ...
    if (!s)
      return Logger.error(
        "Error, no existe este usuario en nuestra base de datos"
      );
    if (s) {
          ...
          (c, context) => {
            if (c.guilds.cache.get(context.guild.id)) {
              ...
              if (
                ...
                const descripcionEmbed =
                  context.guild.customMessage.embed.description
                    .toString()
                    .replace("{streamer}", context.s.display_name)
                    .replace(
                      "{link}",
                ...
                if (context.guild.customMessage.embed.thumbnail) {
                  embed.setThumbnail(
                    context.guild.customMessage.embed.thumbnail
                  );
                }
      ...
      async (client, { req_body }) => {
        ...
        if (!player.queue[0]) {
          return new MessageEmbed()
            ...
            .setAuthor(
              `${client.language.QUEUE[6]} ${req_body[1]} ${client.language.QUEUE[7]}`,
        ...
        embed.setAuthor(
          `${client.language.QUEUE[6]} ${req_body[2]} ${
        ...
        switch (result.loadType) {
          ...
          case "PLAYLIST_LOADED": {
            ...
            const e = new MessageEmbed()
              ...
              .setColor(process.env.bot1Embed_Color)
              .addField(c.language.AUTOMIX[5], `${result.playlist.name}`, true)
              .addField(
            ...
            switch (result.loadType) {
              ...
              case "PLAYLIST_LOADED": {
                ...
                const e = new MessageEmbed()
                  ...
                  .setColor(process.env.bot1Embed_Color)
                  .addField(c.language.AUTOMIX[5], `${playlist[0].title}`, true)
                  .addField(
                    c.language.AUTOMIX[6],
                    `\`${playlist.length}\``,
                    true
      ...
      async (client, { req_body }) => {
        ...
        await api
          ...
          .then((radio) => {
            if (!radio[0]) {
              const errorembed = new MessageEmbed()
                .setColor(15548997)
                .setFooter(
                  client.language.RADIO[4] + ". " + client.language.RADIO[5],
                  req_body[5]
                );
              error = errorembed;
            }
          ...
          .catch((e) => {
            ...
            let webhookClient = new simplestDiscordWebhook(
              process.env.errorWebhookURL
            );
            const errorembed2 = new MessageEmbed().setColor(15548997).setFooter(
              "Error en el comando webhook.js (1)",
              client.user.displayAvatarURL({
                dynamic: true,
              })
            );
        ...
        let embed = await client.manager
          .search(str, req_body[8])
          .then(async (res) => {
            switch (res.loadType) {
              case "TRACK_LOADED":
                player.queue.add(res.tracks[0]);
  ...
  await client.shard
    .broadcastEval(
      async (client, { req_body }) => {
        ...
        if (
          req_body[2][0] * 1000 >= player.queue.current.length ||
          req_body[2][0] < 0
        ) {
          const errorembed = new MessageEmbed()
            .setColor(15548997)
    ...
    .broadcastEval(
      async (client, { req_body }) => {
        ...
        const embed = new MessageEmbed()
          .setColor(process.env.bot1Embed_Color)
          .setTitle(client.language.SUCCESSEMBED)
          .setDescription(client.language.STOP[2])
          .setFooter(req_body[3] + "#" + req_body[4], req_body[5]);
        return embed;
      },
      {
