  ...
  async run(client, interaction, args) {
    if (!interaction.member.voice.channel) {
      const errorembed = new MessageEmbed()
        .setColor(15548997)
        .setFooter(client.language.PLAY[1], interaction.member.displayAvatarURL({
          dynamic: true
        }));
    ...
    await interaction.guild.members
      ...
      .catch((e) => {
        const errorembed2 = new MessageEmbed()
          .setColor(15548997)
          .setFooter("Error en el comando radio (1)", client.user.displayAvatarURL({
            dynamic: true
          }));
      ...
      if (bot2Availability || addToQueue2) {
        fetch(`http://${process.env.IP}:${process.env.bot2Port}/api/v1/radio`, {
          ...
          .catch(() => {
            const errorembed2 = new MessageEmbed()
              .setColor(15548997)
              .setFooter("Error en el comando radio (3)", client.user.displayAvatarURL({
                dynamic: true
              }));
            webhookClient.send(errorembed2)
            bot2missing(client, interaction, data, "radio")
        ...
        if (bot3Availability || addToQueue3) {
          fetch(`http://${process.env.IP}:${process.env.bot3Port}/api/v1/radio`, {
            ...
            .catch(() => {
              const errorembed2 = new MessageEmbed()
                .setColor(15548997)
                .setFooter("Error en el comando radio (4)", client.user.displayAvatarURL({
                  dynamic: true
                }));
              webhookClient.send(errorembed2)
              bot3missing(client, interaction, data, "radio")
          ...
          if (bot4Availability || addToQueue4) {
            fetch(
                `http://${process.env.IP}:${process.env.bot4Port}/api/v1/radio`, {
                  method: "POST",
                  body: JSON.stringify(data),
                  headers: {
                    "Content-Type": "application/json"
              ...
              .catch(() => {
                const errorembed2 = new MessageEmbed()
                  .setColor(15548997)
                  .setFooter("Error en el comando radio (5)", client.user.displayAvatarURL({
                    dynamic: true
                  }));
                webhookClient.send(errorembed2)
                bot4missing(client, interaction, data, "radio")
