...
client.on("voiceStateUpdate", async (oldState, newState) => {
    ...
    const embed = new MessageEmbed()
        .setDescription(`${client.language.VOICESTATEUPDATE[1]}${oldState.guild.me.voice.channel.id}${client.language.VOICESTATEUPDATE[2]}${300000 / 60 / 1000}${client.language.VOICESTATEUPDATE[3]}`)
    ...
    if (!voiceMembers || voiceMembers == 0) {
        ...
        if (player) {
            ...
            newPlayer = await client.manager.newPlayer(oldState.guild, oldState.guild.me.voice.channel, player.textChannel);
