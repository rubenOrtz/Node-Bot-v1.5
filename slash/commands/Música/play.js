...
module.exports = class play extends Command {
  ...
  async run(client, interaction, args) {
          ...
          member.voice.channel.members.forEach(listener => {
            switch (listener.user.id) {
              case process.env.bot2id:
                fetch(`http://localhost:${process.env.bot2Port}/api/v1/new_player`, {
                    method: "POST",
              ...
              case process.env.bot3id:
                fetch(`http://localhost:${process.env.bot3Port}/api/v1/new_player`, {
                    method: "POST",
                    body: JSON.stringify(data),
                    headers: {
                      "Content-Type": "application/json",
              ...
              case process.env.bot4id:
                fetch(`http://localhost:${process.env.bot4Port}/api/v1/new_player`, {
                    method: "POST",
                    body: JSON.stringify(data),
                    headers: {
                      "Content-Type": "application/json",
    ...
    if (bot1Availability || addToQueue) {
      fetch(`http://localhost:${process.env.bot1Port}/api/v1/new_player`, {
          method: "POST",
          body: JSON.stringify(data),
          headers: {
            "Content-Type": "application/json",
      ...
      if (bot2Availability || addToQueue2) {
        ...
        if (bot3Availability || addToQueue3) {
          ...
          if (bot4Availability || addToQueue4) {
            fetch(
                `http://localhost:${process.env.bot4Port}/api/v1/new_player`, {
                  method: "POST",
                  body: JSON.stringify(data),
                  headers: {
                    "Content-Type": "application/json"
