                    ...
                    "options": [{
                            ...
                            "options": [{
                                ...
                                },
                                {
                                    "type": 5,
                                    "name": "timestamp",
                                    "description": "Do you want me to show the time when the message was sended in the footer?",
                                    "required": false
                                },
                            ]
                        },
            ...
            case 'guild':
                switch (interaction.options._subcommand) {
                    case 'embed':
                        ...
                        axios.get(`https://api.twitch.tv/helix/users?login=${interaction.options.getString('streamer').toString()}`, {
                            headers: headers,
                        }).then(async (res) => {
                            if (!res.data.data[0]) {
                            ...
                            TwitchModel.findOne({
                                broadcaster_user_id: res.data.data[0].id /*parseInt(req.body.subscription.condition.broadcaster_user_id)*/
                            }).then((s, err) => {
                                if (!s) {
                                    if (res.data.data[0]) {
                                        axios.post(`https://api.twitch.tv/helix/eventsub/subscriptions`, {
                                            ...
                                            TwitchModel.create({
                                                ...
                                                Interacciones: {
                                                    Guilds: [{
                                                        ...
                                                        textChannel: interaction.options.getChannel('channel').toString().replace('<#', '').replace('>', ''),
                                                        customMessage: {
                                                            embed: {
                                                                color: interaction.options.getString('color').toString(),
                                            ...
                                            const tituloEmbed = interaction.options.getString('title').toString().replace('{streamer}', res.data.data[0].display_name)
                                            const descripcionEmbed = interaction.options.getString('description').toString().replace('{streamer}', res.data.data[0].display_name).replace('{link}', `https://twitch.tv/${res.data.data[0].display_name}`)
                                            const embed = new MessageEmbed()
                                                ...
                                                .setImage(`https://static-cdn.jtvnw.net/previews-ttv/live_user_${res.data.data[0].login}-1920x1089.jpg`);
                                            if (interaction.options.getString('footer') && interaction.options.getString('footericon')) {
                                                embed.setFooter({
                                                    text: interaction.options.getString('footer').toString().replace('{streamer}', res.data.data[0].display_name).replace('{link}', `https://twitch.tv/${res.data.data[0].display_name}`),
                                                    iconURL: interaction.options.getString('footericon').toString()
                                                })
                                            } else if (interaction.options.getString('footer') && !interaction.options.getString('footericon')) {
                                                embed.setFooter({
                                                    text: interaction.options.getString('footer').toString().replace('{streamer}', res.data.data[0].display_name).replace('{link}', `https://twitch.tv/${res.data.data[0].display_name}`)
                                                })
                                            }
                                            if (interaction.options.getString('thumbnail')) {
                                                embed.setThumbnail(interaction.options.getString('thumbnail').toString())
                                            }
                                            if (interaction.options.getString('titleurl')) {
                                                embed.setURL(interaction.options.getString('titleurl').toString().replace('{link}', `https://twitch.tv/${res.data.data[0].display_name}`))
                                            }
                                            if (interaction.options.getBoolean('timestamp') == true) {
                                                embed.setTimestamp()
                                            }
                                            interaction.guild.channels.cache.get(interaction.options.getChannel('channel').toString().replace('<#', '').replace('>', '')).send({
                                                embeds: [embed],
                                                content: `Test Message`
                                            });
                                            interaction.editReply("The notification has been created for this guild, we will send in the channel a test notification so u can see how it looks like");
                                        }).catch(e => {
                                    ...
                                    */
                                    if (s.Interacciones.Guilds.findIndex(i => i.id === interaction.guild.id) == -1) {
                                        ...
                                        s.save();
                                        interaction.editReply(`Añadido a la base de datos correctamente, a partir de ahora recibiras las notificaciones en ${interaction.options.getChannel('channel').toString()}`);
                                        ...
                                        if (interaction.options.getString('footer') && interaction.options.getString('footericon')) {
                                            ...
                                            embed.setFooter({
                                                text: interaction.options.getString('footer').toString().replace('{streamer}', res.data.data[0].display_name).replace('{link}')
                                            })
                                        }
                                        if (interaction.options.getString('thumbnail')) {
                                            embed.setThumbnail(interaction.options.getString('thumbnail').toString())
                                        ...
                                        if (s.Interacciones.Guilds[s.Interacciones.Guilds.findIndex(i => i.id === interaction.guild.id)]) {
                                            interaction.editReply("Esta guild ya está registrada en nuestra base de datos con este streamer. ")
                                        }
                                    }
                                }
                            })
                        ...
                        axios.get(`https://api.twitch.tv/helix/users?login=${interaction.options.getString('streamer').toString()}`, {
                            ...
                            TwitchModel.findOne({
                                ...
                                if (s) {
                                    console.log("aqui2")
                                    if (res.data.data[0]) {
                                        console.log(s.Interacciones.Guilds.findIndex(i => i.id === interaction.guild.id))
                                        ...
                                        s.save().then((s) => {
                                            ...
                                            axios.delete(`https://api.twitch.tv/helix/eventsub/subscriptions`, {
                                                ...
                                                interaction.editReply(`Se ha eliminado la subscripción correctamente`)
                                            })
                                        })
                                    }
                                } else if(!s) return
                                ...
                                if (!s) {
                                    if (res.data.data[0]) {
                                        axios.post(`https://api.twitch.tv/helix/eventsub/subscriptions`, {
                                            ...
                                            const message = interaction.options.getString('message').toString().replace('{streamer}', res.data.data[0].display_name).replace('{link}', `https://twitch.tv/${res.data.data[0].display_name}`)
                                            interaction.member.send({
                                                content: message
                                            });
                                            interaction.editReply("The notification has been created, we will send you a private message with a test notification so u can see how it looks like");
                                        }).catch(e => {
                                            client.logger.error(e);
                                            if (e.response.status.status === 409) {
                                    ...
                                    */
                                   console.log(s.Interacciones.Users.findIndex(i => i.id == interaction.member.id))
                                    if (s.Interacciones.Users.findIndex(i => i.id == interaction.member.id) === -1) {
                                        ...
                                        interaction.editReply("Usted ya está registrado para recibir notificaciones privadas de este streamer.")
                                    }
                                }
                            })
                        }).catch(e => {
                                ...
                                if (s) {
                                    if (res.data.data[0]) {
                                        if (s.Interacciones.Users.findIndex(i => i.id === interaction.member.id) != -1) {
                                            ...
                                            interaction.editReply("Usted no está registrado para recibir notificaciones privadas de este streamer.")
                                        }
                                    }
                                } else if(!s) return
                            })
